...
void initialize(int id) {
 ...
 current_server.ch_info.fd = socket(2, SOCK_STREAM, IPPROTO_TCP);
 ...
 int reuse_addr = 1;
 if (setsockopt(current_server.ch_info.fd, 1, 2, &reuse_addr, sizeof(reuse_addr)) < 0) {
 ...
 if (listen(current_server.ch_info.fd, 10) < 0) {
...
void connect_to_server(int id) {
 ...
 server.servers[id].ch_info.fd = socket(2, SOCK_STREAM, IPPROTO_TCP);
...
int get_host_id(struct sockaddr_in addr) {
 ...
 if (!getnameinfo((struct sockaddr *)&addr, len, hbuf, sizeof(hbuf), ((void *)0), 0, 8)) {
  int i = 0;
 ...
 if (read_size > 0) {
  ...
  if (token1 != ((void *)0)) {
   if (!strcmp(token1, "WRITE")) {
    char *token2 = strtok(((void *)0), " ");
    char *token3 = strtok(((void *)0), " ");
    char *token4 = strtok(((void *)0), " ");
    if (token4 != ((void *)0)) {
...
void start() {
 ...
 do { int __d0, __d1; __asm__ __volatile__ ("cld; rep; " "stosq" : "=c" (__d0), "=D" (__d1) : "a" (0), "0" (sizeof (fd_set) / sizeof (__fd_mask)), "1" (&((&localfds)->__fds_bits)[0]) : "memory"); } while (0);

 for (index = 0; index < 8; index++) {
  if (server.servers[index].ch_info.state == CHANNEL_STATE_CONNECTED) {
   printf("Adding %d to fds\n", server.servers[index].ch_info.fd);
   ((void) (((&fds)->__fds_bits)[((server.servers[index].ch_info.fd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((server.servers[index].ch_info.fd) % (8 * (int) sizeof (__fd_mask))))));
   if (maxfd < server.servers[index].ch_info.fd) {
    maxfd = server.servers[index].ch_info.fd;
   }
  }
 }
 ((void) (((&fds)->__fds_bits)[((0) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((0) % (8 * (int) sizeof (__fd_mask))))));
 ...
 while (!done) {
  ...
  localfds = fds;
  fflush(((void *)0));
  int result = select(maxfd+1, &localfds, ((void *)0), ((void *)0), ((void *)0));
  if (result < 0) {
   ...
   int i = 0;
   for (i = 0; i <= maxfd; i++) {
    if (((((&localfds)->__fds_bits)[((i) / (8 * (int) sizeof (__fd_mask)))] & ((__fd_mask) 1 << ((i) % (8 * (int) sizeof (__fd_mask))))) != 0)) {
    if (i == 0) {
     ...
     if (fd > 0) {
      if (add_delete == 0) {
       ...
       ((void) (((&fds)->__fds_bits)[((fd) / (8 * (int) sizeof (__fd_mask)))] &= ~((__fd_mask) 1 << ((fd) % (8 * (int) sizeof (__fd_mask))))));
       if (maxfd == fd)
        maxfd--;
      } else {
     ...
     if (sockfd < 0) {
      ...
      if (id < 0) {
       ...
       ((void) (((&fds)->__fds_bits)[((sockfd) / (8 * (int) sizeof (__fd_mask)))] |= ((__fd_mask) 1 << ((sockfd) % (8 * (int) sizeof (__fd_mask))))));
       if (maxfd < sockfd) {
        maxfd = sockfd;
       }
      }
     ...
     if (process_receive(i) < 0) {
      ...
      ((void) (((&fds)->__fds_bits)[((i) / (8 * (int) sizeof (__fd_mask)))] &= ~((__fd_mask) 1 << ((i) % (8 * (int) sizeof (__fd_mask))))));
      if (maxfd == i)
       maxfd--;
     }
    }
